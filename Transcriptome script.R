############# Transcriptome_analysis using SALMON method##########################################################################################
/salmon-latest_linux_x86_64/bin/salmon quant -i /UCSC_ref_transcriptome/ -l A -1 "sample_1.fq.gz" -2 "sample_2.fq.gz" -o /salmon/Output -p 10 –gcBias --incompatPrior 0 --validateMappings
...
######################## Scaling the data using median row-center the data################

data<-as.matrix(read.table("rld.txt",header=T,row.names=1))
